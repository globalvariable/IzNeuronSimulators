...
bool create_ps_eligibility_for_neuron_group(Network *network, unsigned int layer, unsigned int nrn_grp, double tau_min, double  tau_max, double max_eligibility_min, double max_eligibility_max)
{
	...
	for (i = 0; i < num_of_neurons; i++)
	{
		...
		if (! create_ps_eligibility_for_neuron(neuron, tau_min, tau_max, max_eligibility_min, max_eligibility_max))
...
bool create_ps_eligibility_for_neuron(Neuron* neuron, double tau_min, double tau_max, double max_eligibility_min, double max_eligibility_max)
{
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		if (neuron->syn_list->synapses[i].plastic)
		{
			...
			if (max_eligibility_min < neuron->syn_list->synapses[i].ps_stdp_pre_post->change_amount)
				return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "create_ps_eligibility_for_neuron", "max_eligibility_min < ps_stdp_pre_post->change_amount.");
...
bool create_ps_eligibility_for_synapse(Neuron* neuron , double tau_min, double tau_max, unsigned int synapse, double max_eligibility_min, double max_eligibility_max)
{
	unsigned int i, j;
	...
	if (! neuron->syn_list->synapses[i].plastic)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "STDP", "create_ps_eligibility_for_synapse", "! neuron->syn_list->synapses[i].plastic.");

	neuron->syn_list->synapses[i].ps_eligibility = g_new0(PsEligibility, 1);
	neuron->syn_list->synapses[i].ps_eligibility->decay_rate = -1.0 / ( ( (tau_max-tau_min) * get_rand_number() ) + tau_min ) ;
	if (max_eligibility_min < neuron->syn_list->synapses[i].ps_stdp_pre_post->change_amount)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "create_ps_eligibility_for_synapse", "max_eligibility_min < ps_stdp_pre_post->change_amount.");
	neuron->syn_list->synapses[i].ps_eligibility->max_eligibility = (max_eligibility_max-max_eligibility_min) * get_rand_number() + max_eligibility_min ;
	neuron->syn_list->synapses[i].ps_eligibility->ps_pol_vals = g_new0(double, parker_sochacki_max_order+1);
	neuron->syn_list->synapses[i].ps_eligibility->decay_rate_ps_pol_vals = g_new0(double, parker_sochacki_max_order+1);
	for (j = 0; j < parker_sochacki_max_order + 1; j++)
...
bool submit_new_ps_eligibility_vals_for_neuron(Neuron* neuron, double tau_min, double tau_max, double max_eligibility_min, double max_eligibility_max)
{
	unsigned int i, j, num_of_synapses;
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		if (neuron->syn_list->synapses[i].plastic)
		{
			neuron->syn_list->synapses[i].ps_eligibility->decay_rate = -1.0 / ( ( (tau_max-tau_min) * get_rand_number() ) + tau_min ) ;
			if (max_eligibility_min < neuron->syn_list->synapses[i].ps_stdp_pre_post->change_amount)
				return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "create_ps_eligibility_for_synapse", "max_eligibility < ps_stdp_pre_post->change_amount.");
			neuron->syn_list->synapses[i].ps_eligibility->max_eligibility = (max_eligibility_max-max_eligibility_min) * get_rand_number() + max_eligibility_min ;
			for (j = 0; j < parker_sochacki_max_order + 1; j++)
			{
				neuron->syn_list->synapses[i].ps_eligibility->decay_rate_ps_pol_vals[j] = neuron->syn_list->synapses[i].ps_eligibility->decay_rate/(j+1);	
...
bool submit_new_ps_eligibility_vals_for_synapse(Neuron* neuron, double tau_min, double tau_max, unsigned int synapse, double max_eligibility_min, double max_eligibility_max)
{
	unsigned int i, j;
	...
	if (max_eligibility_min < neuron->syn_list->synapses[i].ps_stdp_pre_post->change_amount)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "submit_new_ps_eligibility_vals_for_synapse", "max_eligibility < ps_stdp_pre_post->change_amount.");
...
bool submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(Neuron* neuron, Network *axon_from_network, unsigned int axon_from_layer, unsigned int axon_from_nrn_grp, double tau_min, double  tau_max, double max_eligibility_min, double max_eligibility_max)
{
	unsigned int i, j, num_of_synapses;
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		...
		neuron->syn_list->synapses[i].ps_eligibility->decay_rate = -1.0 / ( ( (tau_max-tau_min) * get_rand_number() ) + tau_min ) ;
		if (max_eligibility_min < neuron->syn_list->synapses[i].ps_stdp_pre_post->change_amount)
			return print_message(BUG_MSG ,"IzNeuronSimulators", "Eligibility", "submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group", "max_eligibility_min < ps_stdp_pre_post->change_amount.");
		neuron->syn_list->synapses[i].ps_eligibility->max_eligibility = (max_eligibility_max-max_eligibility_min) * get_rand_number() + max_eligibility_min ;
