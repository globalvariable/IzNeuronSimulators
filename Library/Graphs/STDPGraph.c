...
STDPGraphScrollLimited* allocate_stdp_graph_scroll_limited(GtkWidget *hbox, STDPGraphScrollLimited *graph, unsigned int num_of_data_points, TimeStamp sampling_interval, unsigned int num_of_data_points_to_scroll, TimeStamp buffer_followup_latency, unsigned int num_of_markers, TrialStatusEvents *trial_status_events, STDPBufferLimited* limited_stdp_buffer, unsigned int limited_stdp_buffer_list_idx)
{
	...
	if (graph != NULL)
	{
		print_message(ERROR_MSG ,"IzNeuronSimulators", "STDPGraph", "allocate_stdp_graph_scroll_limited", "graph != NULL");	
		return graph;
	}
	
	graph = g_new0(STDPGraphScrollLimited,1);
	...
	for (i = 0; i < num_of_markers; i++)
	{
		if (! get_status_marker_color(&color_status_marker, i))
			return (STDPGraphScrollLimited*)print_message(ERROR_MSG ,"IzNeuronSimulators", "STDPGraph", "allocate_stdp_graph_scroll_limited", "! get_status_marker_color().");
	...
	graph->limited_stdp_buffer = limited_stdp_buffer;
	graph->limited_stdp_buffer_list_idx = limited_stdp_buffer_list_idx;
...
bool determine_stdp_graph_scroll_limited_start_indexes(STDPGraphScrollLimited *graph, TimeStamp current_system_time, TimeStamp last_sample_time, unsigned int stdp_limited_buffer_write_idx, unsigned int stdp_limited_buffer_size)
{
	...
	if (stdp_limited_buffer_write_idx == 0)
		last_sample_times_idx = stdp_limited_buffer_size - 1;
	else
		last_sample_times_idx = stdp_limited_buffer_write_idx -1;

	if ((current_system_time-last_sample_time) < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "STDPGraph", "determine_stdp_graph_scroll_limited_start_indexes", "(current_system_time-last_sample_time) < 0");
	if ((((current_system_time-last_sample_time)/graph->sampling_interval) + last_sample_times_idx) > stdp_limited_buffer_size)
		current_system_times_idx = ((current_system_time-last_sample_time)/graph->sampling_interval)  + last_sample_times_idx - 	stdp_limited_buffer_size;
	else
...
bool handle_limited_stdp_graph_scrolling_and_plotting(STDPGraphScrollLimited *graph, TimeStamp current_system_time)
{
	...
	STDPBufferLimited* limited_stdp_buffer = graph->limited_stdp_buffer;
	...
	if ((!graph->locally_paused) && (!graph->globally_paused))
	{
		if (graph->scroll_request)   // it is necessary otherwise set_total_limits cannot display slided and clear graph part. 
			scroll_limited_stdp_graph(graph);
		...
		{
			selection_list_idx = graph->limited_stdp_buffer_list_idx;
			buffer_size = limited_stdp_buffer->selected_stdp[selection_list_idx].buffer_size;
			...
			while (idx != end_idx)
			{
				graph->y[graph_old_part_end_idx + graph_sample_idx] = limited_stdp_buffer->selected_stdp[selection_list_idx].stdp[idx];
				graph_sample_idx++;
			...
			set_total_limits_limited_stdp_graph_scroll(graph);
