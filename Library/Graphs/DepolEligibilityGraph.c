#include "DepolEligibilityGraph.h"



DepolEligibilityGraphScrollLimited* allocate_depol_eligibility_graph_scroll_limited(GtkWidget *hbox, DepolEligibilityGraphScrollLimited *graph, unsigned int num_of_data_points, TimeStamp sampling_interval, unsigned int num_of_data_points_to_scroll, TimeStamp buffer_followup_latency, unsigned int num_of_markers, TrialStatusEvents *trial_status_events, DepolEligibilityBufferLimited* limited_depol_eligibility_buffer, unsigned int limited_depol_eligibility_buffer_list_idx)
{
	unsigned int i;
	if (graph != NULL)
	{
		print_message(ERROR_MSG ,"IzNeuronSimulators", "DepolEligibilityGraph", "allocate_depol_eligibility_graph_scroll_limited", "graph != NULL");	
		return graph;
	}
	
	graph = g_new0(DepolEligibilityGraphScrollLimited,1);
	...
	for (i = 0; i < num_of_markers; i++)
	{
		if (! get_status_marker_color(&color_status_marker, i))
			return (DepolEligibilityGraphScrollLimited*)print_message(ERROR_MSG ,"IzNeuronSimulators", "DepolEligibilityGraph", "allocate_depol_eligibility_graph_scroll_limited", "! get_status_marker_color().");
	...
	graph->limited_depol_eligibility_buffer = limited_depol_eligibility_buffer;
	graph->limited_depol_eligibility_buffer_list_idx = limited_depol_eligibility_buffer_list_idx;
...
bool determine_depol_eligibility_graph_scroll_limited_start_indexes(DepolEligibilityGraphScrollLimited *graph, TimeStamp current_system_time, TimeStamp last_sample_time, unsigned int depol_eligibility_limited_buffer_write_idx, unsigned int depol_eligibility_limited_buffer_size)
{
	...
	if (depol_eligibility_limited_buffer_write_idx == 0)
		last_sample_times_idx = depol_eligibility_limited_buffer_size - 1;
	else
		last_sample_times_idx = depol_eligibility_limited_buffer_write_idx -1;

	if ((current_system_time-last_sample_time) < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "DepolEligibilityGraph", "determine_depol_eligibility_graph_scroll_limited_start_indexes", "(current_system_time-last_sample_time) < 0");
	if ((((current_system_time-last_sample_time)/graph->sampling_interval) + last_sample_times_idx) > depol_eligibility_limited_buffer_size)
		current_system_times_idx = ((current_system_time-last_sample_time)/graph->sampling_interval)  + last_sample_times_idx - 	depol_eligibility_limited_buffer_size;
	else
...
bool handle_limited_depol_eligibility_graph_scrolling_and_plotting(DepolEligibilityGraphScrollLimited *graph, TimeStamp current_system_time)
{
	...
	DepolEligibilityBufferLimited* limited_depol_eligibility_buffer = graph->limited_depol_eligibility_buffer;
	...
	if ((!graph->locally_paused) && (!graph->globally_paused))
	{
		if (graph->scroll_request)   // it is necessary otherwise set_total_limits cannot display slided and clear graph part. 
			scroll_limited_depol_eligibility_graph(graph);
		...
		{
			selection_list_idx = graph->limited_depol_eligibility_buffer_list_idx;
			buffer_size = limited_depol_eligibility_buffer->selected_depol_eligibility[selection_list_idx].buffer_size;
			...
			while (idx != end_idx)
			{
				graph->y[graph_old_part_end_idx + graph_sample_idx] = limited_depol_eligibility_buffer->selected_depol_eligibility[selection_list_idx].depol_eligibility[idx];
				graph_sample_idx++;
			...
			set_total_limits_limited_depol_eligibility_graph_scroll(graph);
