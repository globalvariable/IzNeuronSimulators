...
bool determine_neuron_dynamics_graph_scroll_limited_start_indexes(NeuronDynamicsGraphScrollLimited *graph, TimeStamp current_system_time, TimeStamp last_sample_time, unsigned int neuron_dynamics_limited_buffer_write_idx, unsigned int neuron_dynamics_limited_buffer_size)
{
	...
	if ((current_system_time-last_sample_time) < 0)
		return print_message(BUG_MSG ,"IzNeuronSimulators", "NeuronDynamicsGraph", "determine_neuron_dynamics_graph_scroll_limited_start_indexes", "(current_system_time-last_sample_time) < 0");
	if ((((current_system_time-last_sample_time)/graph->sampling_interval) + last_sample_times_idx) > neuron_dynamics_limited_buffer_size)
		current_system_times_idx = ((current_system_time-last_sample_time)/graph->sampling_interval)  + last_sample_times_idx - 	neuron_dynamics_limited_buffer_size;
	else
		current_system_times_idx = ((current_system_time-last_sample_time)/graph->sampling_interval)  + last_sample_times_idx;
	graph->new_part_start_time = current_system_time;
	graph->new_part_start_idx = current_system_times_idx;
