#include "PSPrePostSTDPEligDepol.h"
...
bool evaluate_neuron_dyn_pre_post_stdp_elig_depol(Neuron *nrn, TimeStamp start_time, TimeStamp end_time, bool *spike_generated, TimeStamp *spike_time)	// Assume that only once a spike can be generated per step (0.25 ms).
{			
...
//		printf("event: %llu syn: %u weight: %f type: %u\n", event_time, event_item->syn_idx, synapses[event_item->syn_idx].weight, event_item->event_type); 	
		if (event_item->event_type == NEURON_EVENT_TYPE_SYNAPTIC_EVENT)
		{
			...
			if (synapse->plastic)
			{
				...
				synapse->ps_eligibility->now -= synapse->ps_eligibility->depol_eligibility;
			}
		}
...
static void parker_sochacki_step_for_synapses_w_spike (Neuron *nrn, double dt)
{
	...
	for (i = 0; i < num_of_synapses; i++)
	{
		if (synapses[i].plastic)
		{
			...
			if ((synapse->ps_stdp_pre_post->now + now) > max_eligibility)
				synapse->ps_eligibility->now = max_eligibility;
			else
				synapse->ps_eligibility->now += synapse->ps_stdp_pre_post->now;
		}
	}
	return;
}
