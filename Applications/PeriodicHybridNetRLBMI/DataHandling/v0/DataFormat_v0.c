...
static int load_main_meta_file(char *path_chooser, Network *in_silico_network, Network *blue_spike_network, HybridNetRLBMIData *bmi_data)
{
	...
	double eligibility_decay_rate, max_eligibility;
	bool first;
	...
	bmi_data->reward_data.learning_rate = learning_rate;
			...
			for (k_iter = 0; k_iter < neuron_count; k_iter++)
			{
				...
				for (m = 0; m < num_of_synapses; m++)
				{
					...
					if (plastic)
					{
						...
						if (! create_ps_eligibility_for_synapse(this_neuron, -1.0/eligibility_decay_rate, -1.0/eligibility_decay_rate, m, max_eligibility, max_eligibility))
 							{ fclose(fp); return print_message(ERROR_MSG ,"HybridNetRLBMI", "DataFormat_v0", "load_main_meta_file", "! create_ps_pre_post_stdp_for_synapse."); }
					}
				}
			}		
