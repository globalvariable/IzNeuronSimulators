...
static void submit_num_of_currents_button_func(void)
{
	...
	spike_gen_data->firing_rate_data = allocate_three_phase_poisson_firing_data(spike_gen_data->network, trial_hand_paradigm, spike_gen_data->firing_rate_data);
...
static void apply_firing_rate_to_all_button_func(void)
{
	...
	unsigned int i, j, k;
	if (! layer_neuron_group_neuron_get_selected(combos_select_neuron, &layer_num, &nrn_grp_num, &nrn_num))
		return (void)print_message(ERROR_MSG ,"BMISimulationSpikeGenerator", "CurrentPatternDesignView", "apply_firing_rate_to_all_button_func", "! layer_neuron_group_neuron_get_selected().");
	for (i = 0; i < spike_gen_data->network->layer_count; i++)
	{ 
		for (j = 0; j < spike_gen_data->network->layers[i]->neuron_group_count; j++)
		{
			firing_data->trial_start_available_firing.rates[i][j][0] = atof(gtk_entry_get_text(GTK_ENTRY(entry_firing_rate)));
			firing_data->in_refractory_firing.rates[i][j][0] = atof(gtk_entry_get_text(GTK_ENTRY(entry_firing_rate)));
			for (k = 0; k < spike_gen_data->trial_hand_paradigm->num_of_robot_target_positions; k++)
				firing_data->in_trial_firing[k].rates[i][j][0] = atof(gtk_entry_get_text(GTK_ENTRY(entry_firing_rate)));
		}
	}
}
