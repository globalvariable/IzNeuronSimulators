...
bool create_blue_spike_spike_pattern_view_gui(void)
{
	...
	blue_spike_spike_pattern_graph = allocate_network_spike_pattern_graph_scroll(bmi_data->blue_spike_network, hbox, blue_spike_spike_pattern_graph, 3000, 1000000, 900, 1000, 100000000, bmi_data->blue_spike_spike_data,  NUM_OF_STATUS_MARKERS, bmi_data->trials_data);    // 3 seconds, 1000 samples/sec, 100 ms latency
...
static void pause_button_func(void)
{
	if (blue_spike_spike_pattern_graph->paused)
	{
	...
	{
		blue_spike_spike_pattern_graph->paused = TRUE;
