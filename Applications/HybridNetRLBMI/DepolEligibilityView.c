...
bool create_depol_eligibility_view_gui(void)
{
	...
	HybridNetRLBMIData *bmi_data = get_hybrid_net_rl_bmi_data();
        frame = gtk_frame_new ("");
        frame_label = gtk_label_new ("     DepolEligibility     ");      
	...
	btn_pause_arr = g_new0(GtkWidget*, NUM_OF_DEPOL_ELIGIBILITY_GRAPHS);
	btn_select_arr = g_new0(GtkWidget*, NUM_OF_DEPOL_ELIGIBILITY_GRAPHS);
	combos_select_synapse_arr = g_new0(LayerNrnGrpNrnSynapseCombo*, NUM_OF_DEPOL_ELIGIBILITY_GRAPHS);
	depol_eligibility_graph_arr = g_new0(DepolEligibilityGraphScrollLimited*, NUM_OF_DEPOL_ELIGIBILITY_GRAPHS);
	...
	for (i = 0; i < NUM_OF_DEPOL_ELIGIBILITY_GRAPHS; i++)
	{
		...
		if(!update_texts_of_synapse_combos_when_add_remove(combos_select_synapse_arr[i], bmi_data->in_silico_network))
			return print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "create_depol_eligibility_view_gui", "! update_texts_of_combos_when_add_remove().");	
		...
		depol_eligibility_graph_arr[i] = allocate_depol_eligibility_graph_scroll_limited(hbox, depol_eligibility_graph_arr[i], GRAPH_LENGTHS/PARKER_SOCHACKI_INTEGRATION_STEP_SIZE, PARKER_SOCHACKI_INTEGRATION_STEP_SIZE, GRAPH_SCROLL_LENGTHS/PARKER_SOCHACKI_INTEGRATION_STEP_SIZE, BUFFER_FOLLOWUP_LATENCY, NUM_OF_STATUS_MARKERS, bmi_data->trial_status_events, bmi_data->depol_eligibility_limited_buffer, i);  // 100 ms latency
...
static void combos_select_synapse_func(GtkWidget *changed_combo)
{
	...
	if (i == NUM_OF_DEPOL_ELIGIBILITY_GRAPHS)
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "combos_select_neuron_func", "i == NUM_OF_GRAPHS.");			
	if(!update_texts_of_synapse_combos_when_change(combos_select_synapse_arr[i], bmi_data->in_silico_network, changed_combo))
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "combos_select_neuron_func", "! update_texts_of_combos_when_change().");			
}
...
static void pause_button_func (GtkWidget *btn_pause)
{
	...
	if (i == NUM_OF_DEPOL_ELIGIBILITY_GRAPHS)
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "pause_button_func", "i == NUM_OF_GRAPHS.");	
	if (depol_eligibility_graph_arr[i]->locally_paused)
	{
		if (send_depol_eligibility_graph_resume_request_to_buffer_view_handler(i))   // should resume all graphs at the same time to provide sync
			gtk_button_set_label (GTK_BUTTON(btn_pause_arr[i]),"R");  
	}
	else
	{
		if (is_buffer_view_handler_paused())
			depol_eligibility_graph_arr[i]->locally_paused = TRUE;
		else
			depol_eligibility_graph_arr[i]->local_pause_request = TRUE;
...
static void select_button_func (GtkWidget *btn_select)
{
	...
	if (i == NUM_OF_DEPOL_ELIGIBILITY_GRAPHS)
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "select_button_func", "i == NUM_OF_DEPOL_ELIGIBILITY_GRAPHS.");
	if (! layer_neuron_group_neuron_synapse_get_selected(combos_select_synapse_arr[i], &layer_num, &nrn_grp_num, &nrn_num, &syn_num))
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "select_button_func", "! layer_neuron_group_neuron_synapse_get_selected().");	
	if (!submit_selected_synapse_to_depol_eligibility_buffer_limited(bmi_data->in_silico_network, bmi_data->depol_eligibility_limited_buffer, layer_num, nrn_grp_num, nrn_num, syn_num, i))
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "DepolEligibilityView", "select_button_func", "! submit_selected_neuron_to_depol_eligibility_buffer_limited().");	
	return;
}

unsigned int get_num_depol_eligibility_graphs_w_scroll(void)
{
	return  NUM_OF_DEPOL_ELIGIBILITY_GRAPHS;
}

DepolEligibilityGraphScrollLimited** get_depol_eligibility_graphs_w_scroll_ptr(void)
{
	return depol_eligibility_graph_arr;
}
