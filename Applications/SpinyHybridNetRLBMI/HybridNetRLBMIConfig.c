...
//	double I_inject;
	...
	if (!add_iz_neurons_to_layer(bmi_data->in_silico_network, 20, LAYER_BASE_SERVO_EXTENSOR_SPINY, a, b, c, d, k, C, v_resting, v_threshold, v_peak, inhibitory, E_excitatory, E_inhibitory, tau_excitatory, tau_inhibitory, randomize_params))
	...
	get_neuron_type_parameters(NRN_TYPE_MEDIUM_SPINY, &v, &a, &b, &c, &d, &k, &C, &v_resting, &v_threshold, &v_peak, &inhibitory, &E_excitatory, &E_inhibitory, &tau_excitatory, &tau_inhibitory);
	if (!add_iz_neurons_to_layer(bmi_data->in_silico_network, 20, LAYER_BASE_SERVO_FLEXOR_SPINY, a, b, c, d, k, C, v_resting, v_threshold, v_peak, inhibitory, E_excitatory, E_inhibitory, tau_excitatory, tau_inhibitory, randomize_params))
		return print_message(ERROR_MSG ,"HybridNetRLBMI", "HybridNetRLBMIConfig", "prepare_external_and_in_silico_network", "! add_iz_neurons_to_layer().");
...
bool connect_external_to_in_silico_network(HybridNetRLBMIData *bmi_data)
{
	...
	synapse_data->blue_spike_2_in_silico_excitatory_connection_weight_max = 15;
	synapse_data->blue_spike_2_in_silico_excitatory_connection_weight_min = 15;
...
bool connect_medium_spiny_neurons(HybridNetRLBMIData *bmi_data)
{
	...
	synapse_data->spiny_2_spiny_neuron_inhibitory_connection_probability = 0.7;
	synapse_data->spiny_2_spiny_neuron_inhibitory_connection_weight_max = 16;
	synapse_data->spiny_2_spiny_neuron_inhibitory_connection_weight_min = 16;
