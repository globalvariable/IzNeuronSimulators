...
static NetworkSpikePatternGraphScroll *network_spike_pattern_graph = NULL;
...
static NeuronSpikePatternGraphScroll	***neuron_graphs;
...
bool create_spike_pattern_view_gui(void)
{
	...
	network_spike_pattern_graph = allocate_network_spike_pattern_graph_scroll(spike_gen_data->network, hbox, network_spike_pattern_graph, 3000, 1000000, 900, 1000, 100000000);    // 3 seconds, 1000 samples/sec, 100 ms latency
	...
	network_spike_pattern_graph->new_part_start_time = shared_memory->rt_tasks_data.current_system_time;
	...
	data_point_placement_start_idx = network_spike_pattern_graph->num_of_data_points - network_spike_pattern_graph->num_of_data_points_to_scroll;
...
static gboolean timeout_callback(gpointer user_data) 
{
	...
	if (!network_spike_pattern_graph->paused)
	{
		if (network_spike_pattern_graph->scroll_request)   // it is necessary otherwise set_total_limits cannot display slided and clear graph part. 
		{
			scroll_network_spike_pattern_graph(spike_gen_data->network, network_spike_pattern_graph);
			network_spike_pattern_graph->scroll_request = FALSE;
		}	
		...
		new_part_start_time = network_spike_pattern_graph->new_part_start_time;
		new_part_end_time = new_part_start_time + network_spike_pattern_graph->graph_len_to_scroll;
		if (current_time > (new_part_end_time + network_spike_pattern_graph->spike_buffer_followup_latency) )
		{
			while (graph_spike_buffer_read_idx !=  spike_handling_buff_idx_write) 	// first handle graph' s spike buffer
			{
				...
				if (spike_time < new_part_start_time)
					return print_message(BUG_MSG ,"BMISimulationSpikeGenerator", "SpikePatternView", "gboolean timeout_callback", "graph_buffer_spike_time < new_part_start_time");	
			...
			while (blue_spike_spike_buffer_read_idx !=  blue_spike_buff_idx_write) 	// second handle blue spike buffer
			{
				...
				if (spike_time < new_part_end_time)
					...
					write_to_spike_data(graph_spike_handling_buffer, spike_item->mwa_or_layer, spike_item->channel_or_neuron_group, spike_item->unit_or_neuron, spike_time);
				if (spike_time < new_part_start_time)
					return print_message(BUG_MSG ,"BMISimulationSpikeGenerator", "SpikePatternView", "gboolean timeout_callback", "blue_spike_spike_time < new_part_start_time");	
			...
			network_spike_pattern_graph->scroll_request = TRUE;
			network_spike_pattern_graph->new_part_start_time = new_part_end_time; // prepare for next acquisition
		}
		else		// do not plot if current time is not larger than new_part_end_time. Only handle buffers.
		{
			while (graph_spike_buffer_read_idx !=  spike_handling_buff_idx_write) 	// first handle graph' s spike buffer
			{
				spike_item = &(graph_spike_handling_buffer_buff[graph_spike_buffer_read_idx]);
			...
			while (blue_spike_spike_buffer_read_idx !=  blue_spike_buff_idx_write) 	// second handle blue spike buffer
			{
				...
				if (spike_time < new_part_end_time)
					...
					write_to_spike_data(graph_spike_handling_buffer, spike_item->mwa_or_layer, spike_item->channel_or_neuron_group, spike_item->unit_or_neuron, spike_time);
				if (blue_spike_handling_buffer_buff[blue_spike_spike_buffer_read_idx].peak_time < new_part_start_time)	
					return print_message(BUG_MSG ,"BMISimulationSpikeGenerator", "SpikePatternView", "gboolean timeout_callback", "blue_spike_spike_time < new_part_start_time");	
				blue_spike_spike_buffer_read_idx++;
				if (blue_spike_spike_buffer_read_idx == blue_spike_spike_buffer_size)
					blue_spike_spike_buffer_read_idx = 0;		
...
static void pause_button_func (void)
{
	if (network_spike_pattern_graph->paused)
	{
		...
		scroll_network_spike_pattern_graph(spike_gen_data->network, network_spike_pattern_graph);
		network_spike_pattern_graph->scroll_request = FALSE;
		network_spike_pattern_graph->paused = FALSE;
		gtk_button_set_label (GTK_BUTTON(btn_pause),"R");
	}
	else
	{
		network_spike_pattern_graph->paused = TRUE;
