...
static GtkWidget *entry_depol_eligibility_memb_volt_min;
static GtkWidget *entry_depol_eligibility_memb_volt_max;
...
static GtkWidget *btn_prepare_non_plastic_synapses;
...
static void prepare_non_plastic_synapses_button_func(void);
...
///////////////////////////////////////////// SECOND COLUMN  ///////////////////////////////////////////////////////////////
	...
	gtk_entry_set_text(GTK_ENTRY(entry_STDP_pre_post_change_min), "20");
	...
	gtk_entry_set_text(GTK_ENTRY(entry_STDP_pre_post_change_max), "20");	
	...
	gtk_entry_set_text(GTK_ENTRY(entry_STDP_post_pre_change_min), "5");
	...
	gtk_entry_set_text(GTK_ENTRY(entry_STDP_post_pre_change_max), "5");	
	...
	lbl = gtk_label_new("Depol:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE,TRUE,0);
	lbl = gtk_label_new("Min:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
        entry_depol_eligibility_change_min = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_depol_eligibility_change_min , FALSE,FALSE,0);
	gtk_entry_set_text(GTK_ENTRY(entry_depol_eligibility_change_min), "1");
	gtk_widget_set_size_request(entry_depol_eligibility_change_min, 50, 25) ;
	lbl = gtk_label_new("Max:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
        entry_depol_eligibility_change_max = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_depol_eligibility_change_max, FALSE,FALSE,0);
	gtk_entry_set_text(GTK_ENTRY(entry_depol_eligibility_change_max), "1");	
	gtk_widget_set_size_request(entry_depol_eligibility_change_max, 50, 25) ;

  	hbox = gtk_hbox_new(FALSE, 0);
        gtk_box_pack_start(GTK_BOX(vbox),hbox, FALSE,FALSE,0);

	lbl = gtk_label_new("Depol Volt:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
	lbl = gtk_label_new("");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, TRUE,TRUE,0);
	lbl = gtk_label_new("Min:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
        entry_depol_eligibility_memb_volt_min = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_depol_eligibility_memb_volt_min , FALSE,FALSE,0);
	gtk_entry_set_text(GTK_ENTRY(entry_depol_eligibility_memb_volt_min), "-52");
	gtk_widget_set_size_request(entry_depol_eligibility_memb_volt_min, 50, 25) ;
	lbl = gtk_label_new("Max:");
        gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE,FALSE,0);
        entry_depol_eligibility_memb_volt_max = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_depol_eligibility_memb_volt_max, FALSE,FALSE,0);
	gtk_entry_set_text(GTK_ENTRY(entry_depol_eligibility_memb_volt_max), "-52");	
	gtk_widget_set_size_request(entry_depol_eligibility_memb_volt_max, 50, 25) ;
	...
	btn_prepare_non_plastic_synapses = gtk_button_new_with_label("Prepare Non Plastic Synapses");
	gtk_box_pack_start (GTK_BOX (hbox), btn_prepare_non_plastic_synapses, FALSE, FALSE, 0);
...
/////////  GRAPHS  ////////////////////////////////
	...
	g_signal_connect(G_OBJECT(combo_neuron_type), "changed", G_CALLBACK(combo_neuron_type_func), NULL);
    	...
    	g_signal_connect(G_OBJECT(btn_interrogate_network), "clicked", G_CALLBACK(interrogate_network_button_func), NULL);		
      	...
      	g_signal_connect(G_OBJECT(btn_prepare_non_plastic_synapses), "clicked", G_CALLBACK(prepare_non_plastic_synapses_button_func), NULL);
...
//	gtk_widget_set_sensitive(btn_submit_parker_sochacki_params, FALSE);
	...
	gtk_widget_set_sensitive(btn_prepare_non_plastic_synapses, FALSE);	
...
static void submit_stdp_and_eligibility_button_func(void)
{
	...
	double depol_eligibility_memb_volt_min = atof(gtk_entry_get_text(GTK_ENTRY(entry_depol_eligibility_memb_volt_min)));
	double depol_eligibility_memb_volt_max = atof(gtk_entry_get_text(GTK_ENTRY(entry_depol_eligibility_memb_volt_max)));
	...
	if (! create_ps_eligibility_for_neuron_group(bmi_data->in_silico_network, layer, neuron_group, get_maximum_parker_sochacki_order(),  eligibility_tau_max, eligibility_tau_min, depol_eligibility_change_max, depol_eligibility_change_min, max_eligibility, eligibility_rate, depol_eligibility_memb_volt_max, depol_eligibility_memb_volt_min))
	...
	gtk_widget_set_sensitive(btn_prepare_non_plastic_synapses, TRUE);	
	return;
}
...
static void submit_new_stdp_and_eligibility_for_neuron_button_func(void)
{
	...
	if (! submit_new_ps_eligibility_vals_for_neuron(neuron, get_maximum_parker_sochacki_order(),  eligibility_tau_max, eligibility_tau_min, depol_eligibility_change_max, depol_eligibility_change_min, max_eligibility, eligibility_rate,  depol_eligibility_memb_volt_max, depol_eligibility_memb_volt_min))
...
static void submit_new_stdp_and_eligibility_for_synapse_button_func(void)
{
	...
	if (! submit_new_ps_eligibility_vals_for_synapse(neuron, get_maximum_parker_sochacki_order(),  eligibility_tau_max, eligibility_tau_min, syn_num, depol_eligibility_change_max, depol_eligibility_change_min, max_eligibility, eligibility_rate, depol_eligibility_memb_volt_max, depol_eligibility_memb_volt_min))
		return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "submit_new_stdp_and_eligibility_for_neuron_button_func", "! submit_new_ps_eligibility_vals_for_neuron().");
	...
	while (get_next_neural_net_2_gui_msg_buffer_item(static_msgs_neural_net_2_gui, &msg_item))
	{
		switch (msg_item.msg_type)
		{
			...
			case NEURAL_NET_2_GUI_MSG_UPDATE_SYNAPTIC_WEIGHTS:
				...
				learning_rate = static_bmi_data->learning_rate;
				for (i = 0; i < static_num_of_neurons; i++)
				{
					if (! update_neuron_synaptic_weights_with_history(static_all_neurons[i], reward, learning_rate)) {
...
static void prepare_non_plastic_synapses_button_func(void)
{
	...
	for (i = 0; i < in_silico_network->layers[LAYER_BASE_SERVO_EXTENSOR_SPINY]->neuron_groups[0]->neuron_count; i++)
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_EXTENSOR_SPINY, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_FLEXOR_SPINY, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_FLEXOR_SPINY, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	}	
	for (i = 0; i < in_silico_network->layers[LAYER_BASE_SERVO_FLEXOR_SPINY]->neuron_groups[0]->neuron_count; i++)
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_FLEXOR_SPINY, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_EXTENSOR_SPINY, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_EXTENSOR_SPINY, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	}	

	for (i = 0; i < in_silico_network->layers[LAYER_BASE_SERVO_EXTENSOR_BABBLE]->neuron_groups[0]->neuron_count; i++)
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_EXTENSOR_BABBLE, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_POISSON_NEURONS, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_POISSON_NEURONS, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	}	
	for (i = 0; i < in_silico_network->layers[LAYER_BASE_SERVO_FLEXOR_BABBLE]->neuron_groups[0]->neuron_count; i++)
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_FLEXOR_BABBLE, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_POISSON_NEURONS, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_POISSON_NEURONS, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	...
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_EXTENSOR_SPINY, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_EXTENSOR_BABBLE, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_EXTENSOR_BABBLE, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	}	
	for (i = 0; i < in_silico_network->layers[LAYER_BASE_SERVO_FLEXOR_SPINY]->neuron_groups[0]->neuron_count; i++)
	{
		neuron = get_neuron_address(in_silico_network, LAYER_BASE_SERVO_FLEXOR_SPINY, 0, i);					
		if (! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_FLEXOR_BABBLE, 0, get_maximum_parker_sochacki_order(), 0, 0, 10, 10, 0, 0, 10, 10))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_stdp_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
		if (! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group(neuron, in_silico_network, LAYER_BASE_SERVO_FLEXOR_BABBLE, 0, get_maximum_parker_sochacki_order(), 10, 10, 0, 0, 1, 0, -50, -50))
			return (void)print_message(ERROR_MSG ,"HybridNetRLBMI", "NetworkView", "prepare_non_plastic_synapses_button_func", "! submit_new_ps_eligibility_vals_for_neuron_according_to_pre_synaptic_neuron_group().");
	}	
